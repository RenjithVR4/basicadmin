var adminlistproducts={
	init:function(){
		this.listProducts();
		this.paginationEvent();
                this.productEvent();
		this.editClickEvent();
		this.blockSearchSubmit();
	},
	filterPage:1,
	filterTitle:"",
        filterCategoryTitle:"",
        filterSubcategoryTitle:"",
	filterStatus:"",
	listProducts:function(){
		var _this=this;
		showLoadingDiv(".container");
		$.ajax({
			type:"GET",
			url:"./admin/product.php/products",
			dataType: 'json',
			success:function(data){
				stopLoadingDiv(".container");
                                console.log(data);
				if(!data.error)
				{
					_this.renderProducts(data);
				}
				else
				{
					$("#content-body").html("<tr><td colspan='11'>"+data.error+"</td></tr>");
				}

			},
			error:function(data){
				stopLoadingDiv(".container");
				$("#content-body").html("<tr><td colspan='11'>No product available.</td></tr>");
			}
		});
	},
	renderProducts:function(data){
		_this=this;
		var obj = data;
                console.log(obj.length);

		if(obj.length < 15)
            	{
                        $(".next").addClass("disabled");
            	}
                else
            	{
            		$(".next").removeClass("disabled");
            	}

		var testcontent = "";
		linkedit = '<a class="anchoredit" href="#">';

		var fstatus = '';

		var row='<tr id={idproduct} style="word-break:break-all">'+
		'<td>{title}</a></td>'+
		'<td>{categorytitle}</a></td>'+
                '<td>{subcategorytitle}</a></td>'+
		'<td>{status}</a></td>'+
                // '<td>{created}</a></td>'+
		'<td>'+linkedit+'Edit</a></td>' +
		"</tr>";

		for(i in obj)
		{
			str=row;

			for(j in obj[i])
			{
				thisvalue=obj[i][j];

				if(thisvalue===null)
				thisvalue="Not Defined";

				if(obj[i]['status'] == 'YES')
				{
					fstatus = 'ACTIVE';
				}

				if(obj[i]['status'] == 'NO')
				{
					fstatus = 'INACTIVE';
				}

				var test = "{"+j+"}";
				str=str.replace("{"+j+"}",thisvalue);
				str=str.replace("{status}",fstatus);
			}

			testcontent+=str;
		}

		$("#content-body").html(testcontent);
	},
	productEvent:function(){
		_this=this;
		$(".filter").change(function(){
                        console.log('change');
		      var data = new Object();
		      searchobj = $("#search-form").serializeArray();
                      console.log(searchobj);
		      for(i in searchobj)
		      {
		        	if(searchobj[i].name==="title")
		        		_this.filterTitle=searchobj[i].value;
                                if(searchobj[i].name==="categorytitle")
        		        	_this.filterCategoryTitle=searchobj[i].value;
                                if(searchobj[i].name==="subcategorytitle")
        		        	_this.filterSubcategoryTitle=searchobj[i].value;
		        	if(searchobj[i].name==="status")
		        		_this.filterStatus=searchobj[i].value;
		      }

		      _this.filterPage=1;

		      var filtervalue = new Object();
		      filtervalue['page']=_this.filterPage;
		      filtervalue['title']=_this.filterTitle;
                      filtervalue['categorytitle']=_this.filterCategoryTitle;
                      filtervalue['subcategorytitle']=_this.filterSubcategoryTitle;
		      filtervalue['status']=_this.filterStatus;

		      $(".next").addClass("disabled");
		      $(".previous").addClass("disabled");

		      _this.getProductByFilter(filtervalue);
		  });
	},
	getProductByFilter:function(filterdata)
	{
                console.log(filterdata);
		_this=this;
		showLoadingDiv(".container");
		$.ajax({
		type:"GET",
		url:"./admin/product.php/products",
		data:filterdata,
		success:function(data){
			stopLoadingDiv(".container");
			console.log(data);
			if(!data.error)
			{
				_this.renderProducts(data);
			}
			else
			{
				$("#content-body").html("<tr><td colspan='11'>"+data.error+"</td></tr>");
			}

		},
		error:function(data){
			stopLoadingDiv(".container");
			if(data.status === 401)
				{
					console.log(data.status);
					usermessage("Session Expired","error");
					setTimeout(function (){
					  window.location="index.php";
				  },1000);
				}
			$("#content-body").html("<tr><td colspan='11'>No subcategory available.</td></tr>");
		}
	  });
	},
	paginationEvent:function(){

		_this=this;

		$(".previous").off("click").on("click",function(){

	    	 _this.filterPage--;

	        if(_this.filterPage <= 1)
	        {
	        	$(".previous").addClass("disabled");
	        }

                var filtervalue = new Object();
                filtervalue['page']=_this.filterPage;
                filtervalue['title']=_this.filterTitle;
                filtervalue['categorytitle']=_this.filterCategoryTitle;
                filtervalue['subcategorytitle']=_this.filterSubcategoryTitle;
                filtervalue['status']=_this.filterStatus;

	        _this.getProductByFilter(filtervalue);
	    });

		$(".next").off("click").on("click",function(){

			_this.filterPage++;
	        if(_this.filterPage > 1)
	        {
	        	$(".previous").removeClass("disabled");
	        }

                var filtervalue = new Object();
                filtervalue['page']=_this.filterPage;
                filtervalue['title']=_this.filterTitle;
                filtervalue['categorytitle']=_this.filterCategoryTitle;
                filtervalue['subcategorytitle']=_this.filterSubcategoryTitle;
                filtervalue['status']=_this.filterStatus;

	        _this.getProductByFilter(filtervalue);
	    });
	},
	editClickEvent:function(){
		_this=this;
		$("body").on("click",".anchoredit",function(){
			thisid=$(this).parents("tr").attr("id");
			window.location='editproduct.php?productid='+thisid;

		});
	},
	blockSearchSubmit:function(){
		$("#search-form").submit(function(){
			return false;
		});
	}
}

adminlistproducts.init();
